...
export const gaussianHighPassFilter = async (
  imgName: string,
  ...
  for (let i = 0; i < P; i++)
    for (let j = 0; j < Q; j++) {
      ...
      filterNP.set(i, j, 1 - Math.exp(radius / (2.0 * Math.pow(D0, 2.0))));
    }
  ...
  await cv.imwriteAsync(
    join(global?.__basedir, 'public/dist', `${newFileName}_hg.${fileType}`),
    new cv.Mat(fillterImage, cv.CV_32FC1)
  );
  return `${newFileName}_hg.${fileType}`;
};
